...
\usepackage{colors}   % 色指定
\usepackage{cancel}   % 打ち消し線
...
\tikzset{
  every node/.style={
    ...
    append after command={
      \pgfextra{\let\tikz@transform=\pgfutil@empty}
    }
  }
}
\makeatother
...
\tcbset{
    ...
    claimstyle/.style={
        frame hidden, interior hidden, % 枠と背景を消す
        breakable,
        coltitle=black, fonttitle=\bfseries, % タイトルは太字
        attach title to upper={.\quad},      % タイトルの後にドットと空白を入れて本文開始
        before skip=1ex, after skip=1ex,     % 上下の余白を少し確保
    }
}
